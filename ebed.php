...
$sz_recipes_rand1 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'ebed' or category='leves' order by rand() limit 1");
$sz_recipes_rand2 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'ebed' or category='leves' order by rand() limit 1");
$sz_recipes_rand3 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'ebed' or category='leves' order by rand() limit 1");
$sz_recipes_rand4 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'ebed' or category='leves' order by rand() limit 1");
$sz_recipes_rand5 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'ebed' or category='leves' order by rand() limit 1");
...
<body>
  <nav class="menu" id="mainmenu">
    ...
    <div class="valami">
      ...
      <a href="ebed.php" class="aktiv">Ebéd</a>
  ...
  <main class="tartalom">
    ...
    <article id="munkalap1">
      <article id="munkalap3">
        <h1 style="text-align:center">Ebéd</h1>
        <div>
          <?php
          foreach ($sz_recipes as $sz_recipes) { ?>
            <?php if ($sz_recipes['category'] == 'ebed' || $sz_recipes['category'] == 'leves') { ?>
              <ul>
                <div class="grid">
                  <a href="recipes.php?id=<?= $sz_recipes['id'] ?>"><img style="max-width: 100px" src="media/<?php echo ($sz_recipes['image']) ?>"></a>
