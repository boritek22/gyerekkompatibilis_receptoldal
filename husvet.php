...
$sz_recipes_rand1 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'husvet' order by rand() limit 1");
$sz_recipes_rand2 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'husvet' order by rand() limit 1");
$sz_recipes_rand3 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'husvet' order by rand() limit 1");
$sz_recipes_rand4 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'husvet' order by rand() limit 1");
$sz_recipes_rand5 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'husvet' order by rand() limit 1");
...
<body>
    ...
    <main style="color: black" class="tartalom">
        ...
        <article id="munkalap1husvet">
            <article id="munkalap3husvet">
                <h1 style="text-align:center">Húsvét</h1>

                <div style="margin-left: 2%; display: flex; flex-wrap: wrap;">
                    <?php
                    foreach ($sz_recipes as $sz_recipe) {
                        if ($sz_recipe['category'] == 'husveti') {
