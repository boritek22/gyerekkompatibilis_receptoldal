...
$sz_recipes_rand1 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'halloween' order by rand() limit 1");
$sz_recipes_rand2 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'haloween' order by rand() limit 1");
$sz_recipes_rand3 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'halloween' order by rand() limit 1");
$sz_recipes_rand4 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'halloween' order by rand() limit 1");
$sz_recipes_rand5 = lekerdezes($db, "select * from sz_recipes where image <> '' and category = 'halloween' order by rand() limit 1");
...
<body>
    ...
    <a href="semmi"></a>
    <main style="color: orange" class="tartalom">
        <!-- <div id="munkalap_folott"> -->
        </div>
        <article id="munkalap1halloween">
            <article id="munkalap3halloween">
                ...
                <div style="margin-left: 2%; display: flex; flex-wrap: wrap;">
                    ...
                    foreach ($sz_recipes as $sz_recipe) {
                        if ($sz_recipe['category'] == 'halloweeni') {
