...
$sz_recipes_rands = lekerdezes($db, "select * from sz_recipes where category = 'halloweeni' and sz_recipes.show = '1' order by rand() limit 5");
      ...
      <article id="munkalap3halloween">
        ...
        <?php if (azonositott_e()) {
          ...
          if ($userAdmin == 1) { ?>
            ...
            <h3 style="margin-left: 2%; color: orange">Már engedélyezett receptek</h3>
            <div style="margin-left: 2%; display: flex; flex-wrap: wrap;">
              <?php foreach ($sz_recipes1 as $sz_recipe1) {
                if ($sz_recipe1['category'] == 'halloweeni') {
                  if ($sz_recipe1['image'] != NULL) { ?>
                    <div style="display: flex">
                      <a href="recipes.php?id=<?= $sz_recipe1['id'] ?>"><img onerror="this.onerror=null; this.src='media/default.jpg'" alt="" class="receptkep" style="max-width: 200px" src="media/<?php echo ($sz_recipe1['image']) ?>">
                        <div class="felirathalloween"><?php echo ($sz_recipe1['name']) ?></div>
                      </a>
                    </div>
                  <?php } else { ?>
                    <div style="display: flex">
            ...
            <br>
            <hr style="width: 80%; color: orange; border: 1px solid orange;">
            <h3 style="margin-left: 2%; color: orange;">Még nem engedélyezett receptek</h3>
            <div style="margin-left: 2%; display: flex; flex-wrap: wrap;">
              <?php foreach ($sz_recipes0 as $sz_recipe0) {
                if ($sz_recipe0['category'] == 'halloweeni') {
                  if ($sz_recipe0['image'] != NULL) { ?>
                    <div style="display: flex">
                      <a href="recipes.php?id=<?= $sz_recipe0['id'] ?>"><img onerror="this.onerror=null; this.src='media/default.jpg'" alt="" class="receptkep" style="max-width: 200px" src="media/<?php echo ($sz_recipe0['image']) ?>">
                        <div class="felirathalloween"><?php echo ($sz_recipe0['name']) ?></div>
                      </a>
                    </div>
                  <?php } else { ?>
                    <div style="display: flex">
